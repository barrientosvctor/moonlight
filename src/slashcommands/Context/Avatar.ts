...
.setCallback(async ({ bot, interaction }) => {
  try {
    const member: GuildMember = interaction.guild.members.cache.get(interaction.targetId);
    if (!member) return;

    const embed = new MoonlightEmbedBuilder(interaction.user, interaction.guild!)
    .setDescription(`Avatar de ${member.user.tag}\n[PNG](${member.user.displayAvatarURL({ size: 2048, extension: "png" })}) | [JPG](${member.user.displayAvatarURL({ size: 2048, extension: "jpg" })}) | [WEBP](${member.user.displayAvatarURL({ size: 2048, extension: "webp" })}) ${member.user.avatar !== null ? member.user.avatar.startsWith("a_") ? `[GIF](${member.user.displayAvatarURL({ size: 2048, extension: "gif" })})` : "" : ""}`)
