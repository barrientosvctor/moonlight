...
export default new ContextMenuBuilder()
  ...
  .setCallback(async ({ bot, interaction }) => {
    try {
      ...
      const data = await fetch(`https://discord.com/api/v10/users/${member.user?.id}`, { method: "get", headers: { Authorization: `Bot ${process.env.DISCORD_TOKEN}` } }).then(res => res.json());
