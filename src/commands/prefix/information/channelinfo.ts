...
export default new CommandBuilder({
  ...
  description:
    "Te da meta datos sobre el canal que menciones. Sino daré por hecho que te refieres al canal en el que estás.",
  aliases: ["chinfo"],
  usage: "[#canal | <id>]",
  example: "#general",
  run(client, message, args) {
    const channel =
      getChannel(args[1], message) ||
      (message.channel as GuildTextBasedChannel);
    if (!channel)
      return message.reply(
        client.beautifyMessage("No se pudo encontrar ese canal.", {
          emoji: "error"
        })
      );
