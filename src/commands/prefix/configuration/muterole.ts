  ...
  async run(client, message, args) {
    ...
    if (!args[1])
      return message.reply(
        client.beautifyMessage(
          "Escribe alguna de las opciones para seguir: `set`, `delete` ó `list`",
          { emoji: "noargs" }
        )
      );
    ...
    if (args[1] === "set") {
      if (!args[2])
        return message.reply(
          client.beautifyMessage(
            "Menciona o escribe la ID del rol que usarás para mutear a los usuarios.",
            { emoji: "noargs" }
          )
        );
      ...
      if (!role)
        return message.reply(
          client.beautifyMessage("No se encontró este rol.", { emoji: "error" })
        );
      ...
      return message.reply(
        client.beautifyMessage("Rol establecido éxitosamente.", {
          emoji: "check"
        })
      );
      ...
      if (client.database.has("muterole", message.guildId)) {
        ...
        return message.reply(
          client.beautifyMessage(
            "No tenía establecido anteriormente un rol para ello.",
            { emoji: "error" }
          )
        );

      return message.reply(
        client.beautifyMessage("Rol eliminado éxitosamente.", {
          emoji: "check"
        })
      );
    } else if (args[1] === "list") {
      if (!client.database.has("muterole", message.guildId))
        return message.reply(
          client.beautifyMessage(
            `No hay roles para mostrar. Para establecer uno usa ${inlineCode(`!!${this.name} set <@rol>`)}`,
            { emoji: "error" }
          )
        );
...
- ${roleMention(roleId)} (${inlineCode(roleId)})`);
      ...
      return message.reply(
        client.beautifyMessage(
          "Opción no válida. Las opciones disponibles son las siguientes: `set`, `delete` ó `list`",
          { emoji: "error" }
        )
      );
  }
