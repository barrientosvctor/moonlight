...
export default new CommandBuilder({
  ...
  category: "Moderación",
  description:
    "Banea a algún miembro del servidor. También elimina los mensajes de la última semana.",
  ...
  async run(client, message, args) {
    ...
    try {
      await member.user.send(
        `> ¡Fuiste baneado de ${bold(message.guild.name)}!\nRazón: ${reason}`
      );
    } catch (error) {
    ...
    return message.reply(
      client.beautifyMessage(
        `${bold(member.user.tag)} (${inlineCode(member.user.id)}) fue baneado del servidor.\n${bold("Razón")}: ${reason}`,
        {
          emoji: "check"
        }
      )
