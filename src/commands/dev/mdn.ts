...
export default new CommandBuilder({
  ...
  async run(client, message, args) {
    if (!args[1]) return message.channel.send(client.beautifyMessage("Escribe lo que quieres buscar en la documentación.", { mention: message.author.username, emoji: "noargs" }));
    ...
    for (const doc of data.documents) {
      msg.push(`> ${hyperlink(doc.title, `https://developer.mozilla.org${doc.mdn_url}`)}`);
    }
