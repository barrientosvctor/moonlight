...
export default new CommandBuilder({
  ...
  async run(client, message, args) {
    ...
    if (!args[1])
      return message.channel.send(
        client.beautifyMessage(
          "escriba la ID del usuario para quitarle su ban.",
    ...
    if (user === message.author)
      return message.reply(
        client.beautifyMessage("No puedes quitarte el ban a ti mismo.", {
          emoji: "error"
        })
      );
    if (user === message.guild.members.me?.user)
      return message.reply(
    ...
    try {
      await message.guild.members.unban(
        user.id,
        `Usuario desbaneado por: ${message.author.tag}`
      );
    } catch (error) {
      console.error(error);
      return message.reply(
        client.beautifyMessage("Este usuario no estaba baneado.", {
          emoji: "error"
        })
      );
    }
    ...
    return message.reply(
      client.beautifyMessage(
        `${bold(user.tag)} (${inlineCode(user.id)}) acaba de ser eliminado de la lista de baneos del servidor.`,
        { emoji: "check" }
      )
    );
  }
