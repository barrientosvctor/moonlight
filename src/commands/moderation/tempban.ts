  ...
  async run(client, message, args) {
    ...
    if (!args[1])
      return message.channel.send(
        client.beautifyMessage(
          "Menciona o escriba la ID del miembro que va a banear temporalmente del servidor.",
    ...
    if (!member)
      return message.reply(
        client.beautifyMessage(
          "Este usuario no está en el servidor. Prueba con otro.",
          { emoji: "error" }
        )
      );
    if (
    ...
    if (member === message.guild.members.me)
      return message.reply(
        client.beautifyMessage("¿Por qué yo?", { emoji: "error" })
      );
    if (member.roles.highest.position >= message.member.roles.highest.position)
      return message.reply(
        client.beautifyMessage(
          "No puedes banear a este miembro debido a que cuenta con un rol igual o superior al tuyo.",
          { emoji: "error" }
        )
      );
    if (!member.manageable)
      return message.reply(
        client.beautifyMessage(
          `No puedo banear a ${member.user.tag} ya que tiene un rol igual o superior al mío.`,
          { emoji: "error" }
        )
      );
    if (!member.bannable)
      return message.reply(
        client.beautifyMessage(`No se puede banear a ${member.user.tag}`, {
          emoji: "error"
        })
      );
    ...
    try {
      ...
      return message.channel.send(
        client.beautifyMessage(
          "Hubo un error al intentar banear al usuario del servidor.",
          { emoji: "warning" }
        )
      );
    }
    ...
    try {
      await member.user.send(
        `> ¡Fuiste baneado temporalmente de ${bold(message.guild.name)} durante ${bold(humanDuration)}!\nRazón: ${reason}`
      );
    } catch (error) {
    ...
    message.reply(
      client.beautifyMessage(
        `${bold(member.user.tag)} (${inlineCode(member.user.id)}) fue baneado temporalmente del servidor durante ${bold(humanDuration)}.\n> Razón: ${reason}`,
        { emoji: "check" }
      )
    );
