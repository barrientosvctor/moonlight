...
export default new CommandBuilder({
  ...
  description: "Te da información sobre el rol que me digas, puedes mencionarlo o escribirme la ID del rol.",
  aliases: ["rinfo"],
  usage: "[@rol | <id>]",
  example: "@Moderator",
  run(client, message, args) {
    if (!args[1])
      return message.reply(client.beautifyMessage("Menciona o escribe la ID de un rol.", { mention: message.author.username, emoji: "noargs" }));

    const role = getRole(args[1], message);

    if (!role)
      return message.reply(client.beautifyMessage("No se pudo encontrar ese rol.", { emoji: "error" }))
