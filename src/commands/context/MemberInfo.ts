import { APIUser, ApplicationCommandType, ContextMenuCommandBuilder, EmbedBuilder, Routes } from "discord.js";
...
export default new ContextMenu({
  ...
  async run(interaction, client) {
    ...
    if (!member) return interaction.reply({ content: client.beautifyMessage("No existe este usuario.", { emoji: "error" }), ephemeral: true });

    const data = (await client.rest.get(Routes.user(member.user.id))) as APIUser;
