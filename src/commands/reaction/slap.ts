...
export default new CommandBuilder({
  ...
  async run(client, message, args) {
    if (!args[1])
      return message.channel.send(
        client.beautifyMessage(
          "debes mencionar a una persona para darle una bofetada.",
          { mention: message.author.username, emoji: "error" }
        )
      );
    ...
    if (member.user.id === message.author.id)
      return message.reply(
        client.beautifyMessage("¿Quieres hacerte daño? ¿estás loco?", {
          emoji: "error"
        })
      );
    ...
    embed
      ...
      .setImage(data.url)
      .setDescription(
        `¡${bold(message.author.username)} le dio una bofetada a ${bold(member.user.username)}!`
      );
