...
export default new CommandBuilder({
  ...
  async run(client, message, args) {
    ...
    if (!args[1]) return message.reply(client.beautifyMessage("Escribe alguna de las opciones para seguir: `set` ó `reset`", { emoji: "noargs" }));
    ...
    if (args[1] === "set") {
      ...
      return message.reply(client.beautifyMessage(`El prefix ahora es: ${bold(args[2])}`, { emoji: "check" }));
