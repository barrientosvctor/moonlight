...
if (!args[1]) return message.channel.send(client.beautifyMessage("escribe el nombre de un país o ciudad para obtener el pronóstico del clima.", { mention: message.author.username, emoji: "noargs" }))
      const data = await fetch(`https://api.popcat.xyz/weather?q=${encodeURIComponent(args.slice(1).join(" "))}`).then(res => res.json());
      if (!data) return message.reply(client.beautifyMessage("No encontré esta ubicación.", { emoji: "error" }));
