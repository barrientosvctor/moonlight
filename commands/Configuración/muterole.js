    ...
    async run(bot, msg, args, prefix, getUser, getMember, getRole) {
        try {
            ...
            if(args[1] === 'set') {
                ...
                msg.guild.channels.cache.filter(c => c.type === discord.ChannelType.GuildCategory).forEach(async channel => {
                    await channel.permissionOverwrites.edit(role.id, { 'SendMessages': false, 'AddReactions': false, 'Connect': false, 'Speak': false }).catch(error => msg.channel.send(`${bot.getEmoji('warning')} No se pudo aplicar la configuración de permisos en <#${error.url.split('/')[6]}>. Asegurate que mi rol o el de los bots tenga permitido ver ese canal.`));
                });
                msg.guild.channels.cache.filter(c => c.type === discord.ChannelType.GuildText).forEach(async channel => {
                    await channel.permissionOverwrites.edit(role.id, { 'SendMessages': false, 'AddReactions': false }).catch(error => msg.channel.send(`${bot.getEmoji('warning')} No se pudo aplicar la configuración de permisos en <#${error.url.split('/')[6]}>. Asegurate que mi rol o el de los bots tenga permitido ver ese canal.`));
                });
                msg.guild.channels.cache.filter(c => c.type === discord.ChannelType.GuildVoice).forEach(async channel => {
                    await channel.permissionOverwrites.edit(role.id, { 'Connect': false, 'Speak': false }).catch(error => msg.channel.send(`${bot.getEmoji('warning')} No se pudo aplicar la configuración de permisos en <#${error.url.split('/')[6]}>. Asegurate que mi rol o el de los bots tenga permitido ver ese canal.`));
                });
