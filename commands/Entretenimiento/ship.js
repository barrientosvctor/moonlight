...
module.exports = new Command({
    ...
    async run(bot, msg, args, prefix, getUser) {
	    ...
	    /** @type {discord.User} */
            const user1 = await getUser(args[1]);
	    ...
	    /** @type {discord.User} */
            const user2 = await getUser(args[2]);
            if(!user2) return msg.channel.send(`**${msg.author.username}**, el usuario no existe.`);
            const percent = Math.floor(Math.random() * 101);
            const embed = new discord.EmbedBuilder()
            .setColor('Random')
            .setImage(`https://api.popcat.xyz/ship?user1=${user1.displayAvatarURL({ size: 2048, extension: 'png' })}&user2=${user2.displayAvatarURL({ size: 2048, extension: 'png' })}`)
            .setDescription(`El porcentaje de relación entre **${user1.username}** y **${user2.username}** es de un **${percent}%**. ${bot.shipPercentage(percent)}`);
