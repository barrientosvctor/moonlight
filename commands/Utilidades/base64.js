...
module.exports = new Command({
    ...
    async run(bot, msg, args) {
        try {
            ...
            if(args[1] === 'encode') {
                data = await fetch(`https://some-random-api.ml/base64?encode=${encodeURIComponent(args.slice(2).join(' '))}`).then(res => res.json());
                return msg.reply(`${data.base64}`);
            } else {
                data = await fetch(`https://some-random-api.ml/base64?decode=${encodeURIComponent(args[2])}`).then(res => res.json());
