...
module.exports = new Command({
    ...
    async run(bot, msg, args) {
        try {
            ...
            let data = await fetch(`https://api.popcat.xyz/weather?q=${args.slice(1).join(' ').replace(' ', '%20')}`).then(res => res.json()),
            embed = new discord.MessageEmbed();

            embed.setTitle(`Ubicación: ${data[0].location.name}`)
      ...
      **Zona horaria:** GMT${data[0].location.timezone}`)
            embed.setThumbnail(data[0].current.imageUrl)
