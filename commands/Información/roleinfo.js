...
module.exports = new Command({
    ...
    async run(bot, msg, args, prefix, getUser, getMember, getRole) {
        try {
            ...
            /** @type {discord.Role} */
            let role = getRole(args[1]) || msg.guild.roles.cache.find(r => r.name === args.slice(1).join(' ')),
            embed = new discord.MessageEmbed();
            if(!role) return msg.channel.send(`${bot.getEmoji('error')} Parece que ese rol no pertenece a este servidor.`);
            embed.setColor(role.hexColor)
            embed.setAuthor({ name: `Información del rol [ @${role.name} ]`, iconURL: msg.guild.iconURL({ dynamic: true }) })
            ...
            embed.addField('Permisos', role.permissions.toArray().map(role => `${bot.utils.guild.roles.permissions[role]}`).join(', '));
