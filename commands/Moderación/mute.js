    ...
    async run(bot, msg, args, prefix, getUser, getMember) {
        try {
            ...
            if(db.has(msg.guildId)) {
                ...
                if(member === msg.guild.members.me) return msg.channel.send(`${bot.getEmoji('error')} **${msg.author.username}**, no me puedes mutear del servidor con mis comandos.`);
                ...
                await member.roles.add(await db.get(msg.guildId)).then(async () => {
                    ...
                    setTimeout(async () => {
                        await member.roles.remove(await db.get(msg.guildId)).catch(() => {});
                        try {
